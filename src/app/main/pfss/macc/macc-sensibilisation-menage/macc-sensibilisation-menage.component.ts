...
export class MaccSensibilisationMenageComponent implements OnInit, OnDestroy {
    ...
    toggleForm(setEdit?: boolean) {
        ...
        if (this.show_hide_form === false) {
            ...
            setEdit === true
                ? this.updateSensibilisationForm()
                : this.initializeSensibilisationForm();
        }
    ...
    updateSensibilisationForm() {
        this.sensibilisationForm.patchValue({
            ...
            date_sensibilisation:
                this.selected_sensibilisation[0].date_sensibilisation,
            localite_quartier:
                this.selected_sensibilisation[0].localite_quartier,
            theme_sensibilisation_id:
                this.selected_sensibilisation[0].theme_sensibilisation_id,
            outil_utilise_sensibilisation_id:
                this.selected_sensibilisation[0]
                    .outil_utilise_sensibilisation_id,
            participant_homme:
                this.selected_sensibilisation[0].participant_homme,
            participant_femme:
                this.selected_sensibilisation[0].participant_femme,
            participant_enfant:
                this.selected_sensibilisation[0].participant_enfant,
    ...
    createSensibilisation() {
        ...
        try {
            ...
            this.service.createSensibilisation(data).subscribe((response) => {
            ...
            this.errorSubject.next(
                `Error while creating indicateur: ${error.message}`
            ); // Set error state
            this.isLoadingSubject.next(false); // Set loading state to false when done
        }
    }
    ...
    updateSensibilisation() {
        ...
        try {
            ...
            this.service.updateSensibilisation(data).subscribe((response) => {
                if (response.status === true) {
                    this.show_hide_form = !this.show_hide_form;
                    this.sensibilisationForm.reset();
            ...
            this.errorSubject.next(
                `Error while updating sensibilisation: ${error.message}`
            ); // Set error state
            this.isLoadingSubject.next(false); // Set loading state to false when done
        }
    }
    ...
    deleteSensibilisation() {
        ...
        dialogRef.afterClosed().subscribe((result) => {
            if (result) {
                ...
                this.errorSubject.next(null);
                try {
                    const sensibilisation_id =
                        this.selected_sensibilisation[0].id;
                    ...
                    this.errorSubject.next(
                        `Error while deleting sensibilisation: ${error.message}`
                    );
                    this.isLoadingSubject.next(false);
                }
            }
...
export class MaccSensibilisationMenageService {
    ...
    createSensibilisation(sensibilisation_data: any): Observable<any> {
        ...
        return this.http
            .post(this.end_point_url.apiUrl + "Sensibilisation_macc", data)
            .pipe(
                tap(() =>
                    this.fetchFilteredSensibilisation(
                        +data.data_sensibilisation.village_id
                    ).subscribe()
                )
            );
    }
    ...
    updateSensibilisation(sensibilisation_data: any): Observable<any> {
        ...
        return this.http
            .put(this.end_point_url.apiUrl + "Sensibilisation_macc", data)
            .pipe(
                tap(() =>
                    this.fetchFilteredSensibilisation(
                        +data.updated_sensibilisation.village_id
                    ).subscribe()
                )
            );
    }
    ...
    deleteSensibilisation(
        ...
        return this.http
            .delete(
                this.end_point_url.apiUrl +
                    `Sensibilisation_macc/delete_sensibilisation/${sensibilisation_id}`
            )
            .pipe(
                tap((response) => {
