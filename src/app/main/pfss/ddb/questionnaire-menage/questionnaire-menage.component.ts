...
@Component({
  selector: 'app-questionnaire-menage',
  templateUrl: './questionnaire-menage.component.html',
  styleUrls: ['./questionnaire-menage.component.scss']
})
export class QuestionnaireMenageComponent implements OnInit {
  ...
  onSelectQuestionnaireMenage({ selected }) {
    ...
    save_in_base_questionnaire_menage(etat_suppression) {
        ...
        this.index_api
            .add("liste_variable_menage", this.convertion_data(data), config)
            .subscribe(
                (response) => {
                    if (!this.new_item_questionnaire_menage) {
    ...
    save_in_base_detail_reponse(etat_suppression) {
        ...
        this.index_api
            .add("variable_menage", this.convertion_data(data), config)
            .subscribe(
                (response) => {
                    if (!this.new_item_detail_reponse) {
                        if (etat_suppression == 1) {
